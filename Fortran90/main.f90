...
!
!    19 August 1992.  After I changed the real ( kind = rk8 ) of the matrix
...
!  Variables
!
!    real ( kind = rk8 ) A(MAXROW,MAXEQN), the banded matrix
!    used in NSTOKE and SOLVLIN.  It is stored according to
!    LINPACK/LAPACK general band storage mode.
!
!    real ( kind = rk8 ) AREA(NELEMN), contains the area of each
!    element.
!
!    real ( kind = rk8 ) F(MAXEQN).  After the call to NSTOKE,
...
!    to be dumped for the PLOT3D program.
!
!    real ( kind = rk8 ) G(MAXEQN).  After the call to SOLVLIN,
...
!    program.
!    0, minimal output.
!    1, normal output, plus graphics data files created by GDUMP, UVDUMP
!       and XYDUMP.
!    2, copious output.
!
...
!
!    integer MAXNEW, the maximum number of Newton steps
!    per iteration.
!
!    integer MAXROW, the maximum row real ( kind = rk8 ) of the
!    coefficient matrix that is allowed.
!
...
!    the full system.
!
!    integer NLBAND, the number of diagonals below the main
!    diagonal of the matrix A which are nonzero.
!
...
!    set to 3.
!
!    integer NROW, the used row dimension of the coefficient
!    matrix.
!
...
!
  ...
  real ( kind = rk8 ) a(maxrow,maxeqn)
  real ( kind = rk8 ) a2
  real ( kind = rk8 ) abound
  real ( kind = rk8 ) anew
  real ( kind = rk8 ) aold
  real ( kind = rk8 ) area(nelemn)
  real ( kind = rk8 ) dcda(my)
  real ( kind = rk8 ) f(maxeqn)
  character ( len = 30 ) fileg
  character ( len = 30 ) fileu
  character ( len = 30 ) filex
  real ( kind = rk8 ) g(maxeqn)
...
!
    if ( (icnt == 1.and.jcnt.eq.1).and.(ic /= mx).and.(jc /= my) ) then
...
!
  ...
  ! values: 1=year, 2=month, 3=day, 5=hour, 6=min, 7=sec, 8=ms
  call date_and_time ( values = values )

  ! Salida simplificada: dd/mm/aaaa
  ! values(3)=day, values(2)=month, values(1)=year
  write(*, '("Fecha actual: ", i2.2, "/", i2.2, "/", i4)') &
