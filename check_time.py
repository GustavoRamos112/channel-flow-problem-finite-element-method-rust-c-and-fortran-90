...
def get_nx_ny(filepath: str, re_nx, re_ny) -> tuple[int, int]:
    with open(filepath, "r", encoding="utf-8") as f:
        content = f.read()
    return _extract_int(re_nx.search(content).group(0)), \
           _extract_int(re_ny.search(content).group(0))
...
def benchmark(lang_info: dict, dirpath: str, timeout_s: int = 600) -> list[float] | None:
...
def main():
    ...
    print(f"{'='*70}")
    print(f"Repeticiones por configuracion: {REPETITIONS} (1 warm-up + {MEASURED} medidas)")
    print(f"Configuraciones: {', '.join(f'{lbl} (NX={nx}, NY={ny})' for nx, ny, lbl in CONFIGS)}")
    print()
    ...
    headers = ["Config", "NX", "NY", "Lenguaje", "Media (s)", "Min (s)", "Max (s)", "Compil. (s)"]
    rows = []
    for nx, ny, lbl in CONFIGS:
        for lang_name in LANGUAGES:
            ...
            if times is not None:
                media = sum(times) / len(times)
                rows.append([lbl, nx, ny, lang_name, f"{media:.4f}",
                             f"{min(times):.4f}", f"{max(times):.4f}", f"{t_comp:.2f}"])
            else:
